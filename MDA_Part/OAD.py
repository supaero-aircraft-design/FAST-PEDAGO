 ...
 #Save CSV File
   ...
   'is_sizing': True}
    ...
    def Generate_Input_File_Exo(self):
        ...
        self.INPUT_EXO = oad.generate_inputs(self.CONFIGURATION_FILE_EXO, SOURCE_FILE_EXO, overwrite=True)
...
# GENERATE THE REDESIGN AIRCRAFT DATA FILE
    ...
    def para_payload_range(self,Path,sfc,var_owe=None,var_mtow=None,name=None, fig=None, file_formatter=None, Color=None):
        self.Path=Path
        ...
        if var_mtow ==None:
            ...
            if Point_D> Max_Payload:
                print('Warning! Point C is over Payload_Max, Incremental Development out of limits')
        else:
            FuelD = MFW
            Point_D = MTOW  - OWE - MFW # payload
            if Point_D > Max_Payload:
 ...
 # COMPUTE THE SPECIFIC AIRCRAFT RANGE
    ...
    def Npax_BF_Diagramm(self,Path,sfc,name=None, fig=None, file_formatter=None,Color=None):
        self.Path=Path
        self.sfc_bf=sfc
        self.name=name
        self.BlockFuel = []
        ...
        scatter_Npax_BF_DOC = go.Scatter(x=BF_DOC, y=BF_DOC/Pax_cabin_SPP, name=self.name, line=dict(color=Color))
